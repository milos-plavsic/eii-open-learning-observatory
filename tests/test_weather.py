...
class WeatherTests(unittest.TestCase):
    ...
    def test_key_epochs_are_bound_rotate_with_purge_and_cannot_reuse_secrets(self):
        with tempfile.TemporaryDirectory() as directory:
            ...
            with WeatherStore(
                path, secret=b"0123456789abcdef0123456789abcdef", key_epoch="v1"
            ) as store:
                store.ingest(self.event("one"))
            with self.assertRaisesRegex(ValueError, "different privacy secret"):
                WeatherStore(path, secret=b"fedcba9876543210fedcba9876543210", key_epoch="v1")
            with self.assertRaisesRegex(ValueError, "cannot be reused"):
                WeatherStore(path, secret=b"0123456789abcdef0123456789abcdef", key_epoch="v2")
            with WeatherStore(
                path, secret=b"0123456789abcdef0123456789abcdef", key_epoch="v1"
            ) as rotated:
                self.assertEqual(
    ...
    def test_cli_exports_only_thresholded_cells(self):
        with tempfile.TemporaryDirectory() as directory:
            ...
            self.assertEqual(data["cells"][0]["contributor_count"], 3)
    ...
    def test_exports_are_coarsened_and_rate_limit_differencing(self):
        with tempfile.TemporaryDirectory() as directory:
            ...
            with WeatherStore(
                ...
                self.assertEqual(first["cells"][0]["contributor_count"], 2)
