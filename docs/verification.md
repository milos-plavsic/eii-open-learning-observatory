...
This is a local verification record for the first `0.1.0` source version. It is
...
|---|---|
| Warning-as-error unit/integration suite | 241 tests passed |
| Coverage | 5,626 tracked statements and 1,756 branches at 100%, with 33 declared non-executable protocol and opposite-platform lock lines excluded |
| Lint and formatting | Expanded Ruff rules and formatter passed for `src`, `tests`, and `tools` |
| Static typing | Repository-wide strict `mypy` passed for 71 source files without module exemptions or ignored missing imports |
| Critical mutation probes | Twenty-two mutations spanning cryptography, persistence, PLCT conformance, release/SBOM evidence, scoped safety decisions, replay text integrity, privacy bounds/differencing, model transport, retrieval, split/merge and scoped alignment, semantic abstention, status projection, canonical quotation, snapshot signing, incomplete metering, consensus consistency and uncertainty matching were killed |
...
| Clean source binding | Automated tests prove dirty-tree rejection and commit/archive binding; this working tree must still be committed before release evidence is generated |
| Version binding | Runtime, evidence producer, wheel metadata, source-distribution metadata, and requested candidate version all reported `0.1.0` |
...
PYTHONPATH=src uv run python tools/release_preflight.py --version 0.1.0 DIST/*.whl DIST/*.tar.gz
```
