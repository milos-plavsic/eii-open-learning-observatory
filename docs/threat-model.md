...
| Learner surveillance | Raw fields are rejected; retention purges events; contributions are bounded. Compatibility mode uses cell/day pseudonyms and exact thresholding. Fixed-universe mode uses course/day pseudonyms, bounds cells per contributor, retains zero cells and applies Laplace noise under a persistent composition budget. Blocked/empty releases spend no budget; identical scope/snapshot/policy releases reuse noise; exact bytes are hash-ledgered. | Fixed-universe mode is internally verified, not independently reviewed. It increases within-course/day linkage to enforce contributor adjacency. Compatibility mode remains partial because cell selection is data-dependent. Epsilon and utility need deployment review. |
...
| Privacy-budget database clone | A database instance ID and append-only parent lineage reject silent use under another deployment identity. An explicit break-glass override records a fork. | Fork detection cannot discover two copies both using the original identity. Institution-level inventory, protected backups and conservative cross-fork accounting remain necessary. |
